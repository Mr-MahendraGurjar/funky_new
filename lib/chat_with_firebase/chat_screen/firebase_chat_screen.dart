...
class FirebaseChatScreen extends StatelessWidget {
  ...
  FirebaseChatScreen({super.key, required this.receiverId});
  final TextEditingController _inputController = TextEditingController();
  final controller = Get.put(FirebaseChatController());

  @override
  Widget build(BuildContext context) {
    controller.CreatorgetUserInfo_Email(UserId: receiverId);
    return Stack(
      children: [
        Container(
        ...
        Scaffold(
                      ...
                      children: [
                        ...
                        Obx(() => CircleAvatar(
                              backgroundImage: NetworkImage(
                                  '${URLConstants.base_data_url}images/${controller.userInfoModel_email.value.data?[0].image ?? ''}'),
                            )),
                        const SizedBox(width: 10),
                        Column(
                          ...
                          children: [
                            Obx(
                              () => Text(
                                controller.userInfoModel_email.value.data?[0]
                                        .fullName ??
                                    '',
                            ...
                            StreamBuilder<DocumentSnapshot>(
                                stream:
                                    FirebaseServices.getUserStatus(receiverId),
          ...
          body: Container(
            ...
            child: StreamBuilder<QuerySnapshot>(
                stream: FirebaseServices.getChats(receiverId),
                builder: (context, snapshot) {
                  ...
                  if (chats.isEmpty) {
                    ...
                    return ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          if (chats[index].receiverId ==
                                  CacheHelper.getString(key: 'uId') &&
                              chats[index].isSeen == false) {
                            try {
                              FirebaseServices.updateChat(
                                  senderId: chats[index].senderId!,
                                  docId: chats[index].dcoId);
                            } catch (e) {
                              log('isSeen Error', error: e);
                            }
                          }
                          return buildItem(chats[index]);
                        });
                  }
                }),
          ),
              ...
              children: <Widget>[
                SizedBox(
                  ...
                  child: IconButton(
                      ...
                      onPressed: () async {
                        await FirebaseServices.sendMediaChat(receiverId);
...
class FirebaseChatController extends GetxController {
  ...
  final RxList selectedChatList = [].obs;

  Future<UserInfoModel?> CreatorgetUserInfo_Email(
      {required String UserId}) async {
    String loginId = CacheHelper.getString(key: 'uId');
    print('Login Id::=>$loginId');
    String url =
        ("${URLConstants.base_url}${URLConstants.user_info_email_Api}?id=$UserId&login_user_id=$loginId");
