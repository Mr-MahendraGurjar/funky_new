...
class SelectUsersScreen extends StatefulWidget {
  final Group group;
  const SelectUsersScreen({super.key, required this.group});

  @override
  State<SelectUsersScreen> createState() => _AddUserPageState();
}

class _AddUserPageState extends State<SelectUsersScreen> {
  List image_list = [
    AssetUtils.image1,
    AssetUtils.image2,
    AssetUtils.image3,
  ...
  List<String> selectedUserIds = <String>[];
  @override
      ...
      child: Scaffold(
        appBar: AppBar(
          title: Column(
            ...
            children: [
              Text(
                selectedUserIds.isNotEmpty ? 'Add Users' : 'Select User',
                style: const TextStyle(
                    fontFamily: 'PM', fontSize: 16, color: Colors.white),
              ),
              Visibility(
                visible: selectedUserIds.isNotEmpty,
                child: Text(
                  selectedUserIds.length.toString(),
                  style: const TextStyle(
                      fontFamily: 'PM', fontSize: 16, color: Colors.white),
                ),
              ),
          ...
          actions: [
            InkWell(
              onTap: () async {
                selectedUserIds.add(CacheHelper.getString(key: 'uId'));
                showLoader(context);
                String imageUrl = await FirebaseServices.uploadFile(
                    filePath: widget.group.image ?? "", contentType: '.jpg');
                Group group = Group(
                  name: widget.group.name,
                  image: imageUrl,
                  aboutGroup: widget.group.aboutGroup,
                  members: selectedUserIds,
                  createAt: DateTime.now().toString(),
                  createdBy: CacheHelper.getString(key: 'uId'),
                );
                await FirebaseServices.createGroup(group);
                hideLoader(context);
                Get.back();
              },
              child: const Text("Add",
                  style: TextStyle(
                      fontFamily: 'PM', fontSize: 16, color: Colors.white60)),
            )
          ],
        ),
        key: _globalKey,
                            ...
                            : (_search_screen_controller.searchlistModel == null
                                ...
                                : Expanded(
                                    ...
                                    itemBuilder:
                                      ...
                                      return Container(
                                        color: selectedUserIds.contains(
                                                _search_screen_controller
                                                    .searchlistModel!
                                                    .data![index]
                                                    .id!)
