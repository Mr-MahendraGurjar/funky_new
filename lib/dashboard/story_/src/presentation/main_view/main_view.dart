...
import 'package:funky_new/dashboard/story_/src/presentation/main_view/widgets/rendering_indicator.dart';
...
import '../utils/constants/render_state.dart';
...
class _MainViewState extends State<MainView> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      ...
      var item = Provider.of<DraggableWidgetNotifier>(context, listen: false);

      control.mediaPath = widget.imagedata.path;
      if (control.mediaPath.isNotEmpty) {
        final extension = p.extension(control.mediaPath); // '.dart'
        print(extension);
        if (extension == '.MP4' || extension == '.mp4') {
          item.draggableWidget.insert(
              0,
              EditableItem()
                ..type = ItemType.video
                ..position = const Offset(0.0, 0));

          print("_control.mediaPath : ${control.mediaPath}");
        } else {
          item.draggableWidget.insert(
              0,
              EditableItem()
                  ...
                  child: Stack(
                    children: [
                      ScrollablePageView(
                          ...
                          gallery: const Text('GalleryMediaPicker')
                          //! comment
                          // GalleryMediaPicker(
                          //   // onlyVideos: true,
