...
class HomeCubit extends Cubit<HomeState> {
      ...
      if (data.size != 0) {
        ...
        for (var element in data.docs) {
          if (!users.any((e) => e.id == element.id)) {
            users.add(
                UserModel.fromJsonMap(map: element.data(), uId: element.id));
          }
        }
  ...
  void getUser() {
    FirebaseFirestore.instance.collection(userCollection).get().then((value) {
      if (value.size != 0) {
        for (var element in value.docs) {
          if (!users.any((e) => e.id == element.id)) {
            ...
            users[users.indexWhere((e) => e.id == element.id)] =
                UserModel.fromJsonMap(map: element.data(), uId: element.id);
          }
        }
        emit(SuccessGetUsersState());
  ...
  void getCallHistory() {
    FirebaseFirestore.instance.collection(callsCollection).get().then((value) {
  ...
  Future<void> fireVideoCall({required CallModel callModel}) async {
    ...
    Map<String, dynamic> queryMap = {
      // 'channel_${UniqueKey().hashCode.toString()}'
